...
# 1) 실제 Query 백엔드 수신 URL은 아래 주석 항목에 추가해서 stream 루프에서 POST로 전송하면 됨.
#    EXTERNAL_QUERY_INGEST_URL = "http://<backend-host>:<port>/api/query/telemetry"
# 2) 백엔드가 준비되면 이 서버의 /api/* GET 라우트도 동일 스키마(차량 목록/상세/히스토리)로 리버스 프록시하거나
#    프론트엔드에서 URL을 실제 백엔드 경로로 직접 교체하면 됨.
...
def _load_dummy_data() -> Dict[str, Any]:
    fallback = {
        ...
        "seed_vehicles": [
            ...
            {
                "vehicle_id": "CAR-1002",
                "vin": "KICF9AA1001000002",
                "model": "Tesla Model Y",
                "driver": "이수민",
                "city": "용인",
                "latitude": 37.2411,
                "longitude": 127.1776,
                "odometer_km": 41220.89,
                "battery_soc": 46.3,
                "battery_health": 93.1,
                "trip_state": "IDLE",
                "is_locked": True,
            ...
            {
                "vehicle_id": "CAR-1003",
                "vin": "KICF9AA1001000003",
                "model": "Kia EV6",
                "driver": "박소윤",
                "city": "수원",
                "latitude": 37.2636,
                "longitude": 127.0286,
                "odometer_km": 18670.56,
                "battery_soc": 92.8,
                "battery_health": 99.2,
                "trip_state": "DRIVE",
                "is_locked": False,
...
def build_payload(vehicle: VehicleState, events: List[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "telemetry": {
            ...
            "trip_state": vehicle.trip_state,
            "trip_duration_s": random.randint(0, 7200),
            "battery_soc_pct": round(vehicle.battery_soc, 2),
            "battery_health_pct": round(vehicle.battery_health, 2),
            "cabin_temp_c": round(vehicle.cabin_temp_c, 1),
            "ambient_temp_c": round(vehicle.ambient_temp_c, 1),
            "engine_temp_c": round(vehicle.engine_temp_c, 1),
            "coolant_temp_c": round(vehicle.coolant_temp_c, 1),
            ...
            "voltage_v": round(370 + (vehicle.battery_soc - 50.0) * 0.09, 2),
            "pack_current_a": round((vehicle.speed_kmh * 0.08) * random.uniform(-1.0, 1.0) * 0.1, 2),
            "12v_battery_v": round(random.uniform(12.0, 12.8), 2),
        },
        "status": {
            ...
            "driving_mode": random.choice(["Normal", "Eco", "Sport", "Standard"]),
            "door": {
                "front_left_locked": vehicle.is_locked,
                "front_right_locked": vehicle.is_locked,
                "rear_left_locked": vehicle.is_locked,
                "rear_right_locked": vehicle.is_locked,
            },
            "window": {"front": random.choice(["up", "down"]), "rear": random.choice(["up", "down"])},
        },
        "diagnostics": {
            "tire_pressure_psi": {
...
async def stream_vehicle(vehicle_id: str) -> None:
    ...
    while True:
        ...
        print(
            f"[{payload['timestamp']}] {vehicle_id} | {vehicle.trip_state:<5} | "
            f"{payload['telemetry']['speed_kmh']:>5} km/h | SOC {payload['telemetry']['battery_soc_pct']:.1f}%"
        )
        # TODO(backend-integration): 현재는 console 출력 + history_store 저장만 수행.
        # 실제 운영 시 여기에 HTTP POST(EXTERNAL_QUERY_INGEST_URL, json=payload) 추가.
        print(payload)
...
@app.get("/", response_model=VehicleEnvelope)
def root():
            ...
            result.append(
                {
                    ...
                    "driver": latest.get("driver"),
                    "timestamp": latest.get("timestamp"),
                    "trip_state": latest.get("telemetry", {}).get("trip_state"),
                    "speed_kmh": latest.get("telemetry", {}).get("speed_kmh"),
                    "battery_soc_pct": latest.get("telemetry", {}).get("battery_soc_pct"),
                }
            )
    return {"count": len(result), "vehicles": result}


@app.get("/api/vehicles/{vehicle_id}", response_model=VehicleEnvelope)
